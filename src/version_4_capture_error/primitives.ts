...
import * as PM from "../version_4_capture_error/parser_monad"
import {PReturnObj, ParserResult, Parser, makeFailedParserResult, makeParserResult, eta, bind} from "../version_4_capture_error/parser_monad"
...
export function createPredicateParser(predicate: (ch: string) => boolean): Parser<string> {
    return function(sinput: string){
        if((sinput.length == 0) || (! predicate(sinput.substring(0,1))))
            return makeFailedParserResult(`predicate_parser`, sinput)
        const value = sinput.substring(0,1)
        const remainder = sinput.slice(1)
        return makeParserResult(value, remainder)
    }
}
...
export function parser_or<T,R>(p1: Parser<T>,  p2: Parser<R>): Parser<T|R> {
    return function(sinput: string): ParserResult<T|R> {
        ...
        if(Result.isFailedResult(res1)) {
            const res2 = p2(sinput)
            if(Result.isFailedResult(res2))
                return makeFailedParserResult("choice", sinput)
            else {
                const {result:v2, remaining:r2}:PReturnObj<T|R> = Result.getValue(res2)
                return makeParserResult((v2), r2)
            }
        }
        const {result:v1, remaining:r1}:PReturnObj<T> = Result.getValue(res1)
        return makeParserResult((v1), r1)
    }
}
export const choiceN = parser_or_N
function parser_or_N(ps: Array<Parser<Ast>>, input: string): ParserResult<Ast> {
    if(ps.length == 0) {
        return makeFailedParserResult("choiceN N=0", input)
    }
    ...
    return makeParserResult((r1), rem1)
}
...
function stopRecursionMarker<T>(sinput:string):ParserResult<T[]> {
    return PM.makeParserResult([], sinput)
}
/**
...
//         const r = pp(sinput)
//         if(Result.isFailedResult(r)) {
//             return makeFailedParserResult("create_OneOrMore_2", sinput)
//         } else {
//             const {result: cs, remaining: rem} = Result.getValue(r)
//             return makeParserResult(cs.join(""), rem)
//         }
//     }
// }
...
//         let parse_result = ""
//         if(Result.isFailedResult(mr1)) {
//             return makeFailedParserResult("create_OneOrMore_1", sinput)
//         } else {
//             // const pair = Maybe.get_value(r)
//             const {result: first_digit_as_string, remaining: r1}: PReturnObj<string> = Result.getValue(mr1)
//             // const remain = pair.remaining_input
//             const mr2 = manyTimes(r1)
//             if(Result.isFailedResult(mr2)) {
//                 return makeParserResult(first_digit_as_string, r1)
//             }
//             // const result_pair = Maybe.get_value(r2)
//             const {result: subsequent_digits_as_string, remaining: r2}: PReturnObj<string> = Result.getValue(mr2)
...
//             return makeParserResult(parse_result_string, r2) 
//         }
//     }
// }

export const parseAdditionSign = whitespaceIfy(createPredicateParser((s) => (s === "+")))
export const parseMultiplySign = function(sinput: string): PM.ParserResult<string> {
    return whitespaceIfy(createPredicateParser((s) => (s === "*")))(sinput)
}
...
export function parseNumber(sinput: string): ParserResult<string> {
    ...
    const numParser = create_OneOrMoreParser_3(digitParser)
...
export function followedBy3_new<R,S,T,U>(pr: Parser<R>, ps: Parser<S>, pt: Parser<T>, f:(r: R, s:S, t:T) => U):Parser<U> {
...
export function followedBy3<R,S,T,U>(pr: Parser<R>, ps: Parser<S>, pt: Parser<T>, f:(r: R, s:S, t:T) => U):Parser<U> {
    return function(sinput: string): ParserResult<U> {
        ...
        const {result: rv, remaining: rem1} = Result.getValue(res1)
        const res2 = ps(rem1)
        if(Result.isFailedResult(res2)) {
            return makeFailedParserResult(res2.code, res2.rem)
        }
        const {result: sv, remaining: rem2} = Result.getValue(res2)
        const res3 = pt(rem2)
        if(Result.isFailedResult(res3)) {
            return makeFailedParserResult(res2.code, res3.rem)
        }
        const {result: tv, remaining: rem3} = Result.getValue(res3)

        return makeParserResult(f(rv, sv, tv), rem3)
    } 
}
